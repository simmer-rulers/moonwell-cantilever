...
const sdk = new CoinbaseWalletSDK({
  appName: "Moonwell Cantilever (Built for Base)",
...
async function rpcPulse() {
  ...
  const isBase = reportedChainId === 8453 || reportedChainId === 84532;

  print([
    "RPC Pulse",
    `Network: ${active.label}`,
    `Reported chainId: ${reportedChainId} (${isBase ? "Base-compatible" : "unexpected"})`,
    `Latest block: ${bn}`,
...
function mount() {
  ...
  h1.textContent = "Moonwell Cantilever";

  const blurb = document.createElement("div");
  blurb.textContent =
    "Built for Base: connect wallet, validate chainId, and run read-only ETH + ERC-20 lookups with Basescan references.";
